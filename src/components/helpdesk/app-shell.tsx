...
export function AppShell({ children, title, role = "admin" }: { children: ReactNode; title: string; role?: Role }) {
  ...
  useEffect(() => {
    ...
    else if (user.role !== role) navigate({ to: roleHome[user.role], replace: true });
  }, [ready, user, role, navigate]);
  if (!ready || !user || user.role !== role) return <div className="grid min-h-screen place-items-center bg-background text-sm text-muted-foreground">جارٍ التحقق من الجلسة...</div>;
  ...
  return <div className="min-h-screen bg-background p-0 lg:p-4"><div className="dashboard-shadow mx-auto flex min-h-screen max-w-[1680px] overflow-hidden border-border bg-card lg:min-h-[calc(100vh-2rem)] lg:rounded-lg lg:border">
    <Sidebar role={role} userName={user.name} roleLabel={user.roleLabel} className={cn("fixed inset-y-0 right-0 z-50 transition-transform lg:static lg:translate-x-0", mobileMenu ? "translate-x-0" : "translate-x-full")} close={() => setMobileMenu(false)} />
...
function Sidebar({ className, close, role, userName, roleLabel }: { className?: string; close: () => void; role: Role; userName: string; roleLabel: string }) {
  ...
  return <aside className={cn("flex w-56 shrink-0 flex-col bg-sidebar text-sidebar-foreground", className)}>
    ...
    <div className="space-y-2 border-t border-sidebar-border p-3"><div className="flex items-center gap-2 rounded-md bg-sidebar-accent p-2.5"><div className="grid h-8 w-8 place-items-center rounded-full bg-brand-green text-xs text-primary-foreground">{userName[0]}</div><div className="min-w-0"><p className="truncate text-xs font-bold">{userName}</p><p className="truncate text-[10px] text-sidebar-foreground/60">{roleLabel}</p></div></div><Button variant="ghost" size="sm" className="w-full justify-start text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-foreground" onClick={() => { logout(); navigate({ to: "/login", replace: true }); }}><LogOut className="h-4 w-4" />تسجيل الخروج</Button></div>
  </aside>;
}